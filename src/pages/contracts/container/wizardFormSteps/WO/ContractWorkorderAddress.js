...
const ContractWorkorderAddress = () => {
    ...
    const startDateColumn = (rowData) => {
        return <div>{rowData.startDate ? moment(rowData.startDate).format('MM/DD/YY') : null}</div>;
    };

    const endDateColumn = (rowData) => {
        return <div>{rowData.endDate ? moment(rowData.endDate).format('MM/DD/YY') : 'NA'}</div>;
    };
            ...
            <div>
                ...
                <div className="grid">
                    {countryUi?.addressLines?.map((ui, i) => {
                        return (
                            <div className="md:col-3 sm:col-3">
                                <CustomInputText
                                    control={control}
                                    errors={errors}
                                    name={ui.addressLine}
