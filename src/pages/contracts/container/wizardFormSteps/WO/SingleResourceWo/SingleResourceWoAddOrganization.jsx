...
import { addDynamicData } from '../../../../../../redux/actions/contractActions';

function SingleResourceWoAddOrganization({ control, errors, setValue,setSkip }) {
    const [sidebarVisible, setSidebarVisible] = useState(false);
    const [sidebarVisibleTable, setSidebarVisibleTable] = useState(false);
    const [selectedProduct, setSelectedProduct] = useState(null);
    const sampleDynamicData = useSelector(state => state.contract.dynamicSampleData);
    console.log(sampleDynamicData,'sampleDynamicData');
    const [sample, setSample] = useState(sampleDynamicData);
    ...
    useEffect(() => {
        // Update the local state when sampleDynamicData changes
        setSample(sampleDynamicData);
      }, [sampleDynamicData]);
    
      useEffect(() => {
        dispatch(addDynamicData(sample));
      }, [sample, dispatch]);
      useEffect(()=>{
        setSkip(true);
      },[setSkip]);
    ...
    //     setSample(sampleDynamicData);
    //   }, [sampleDynamicData]);
    // const handleAddSample = (sampleDynamicData) => {
    //     setSample(sampleDynamicData);
    // };
    // const options = [
    ...
    return (
        <>
            <div className="p-fluid mb-6">
                <h4 className='fw-bold text-center'>Add organization(s) if Applicable in the contract Layers</h4>
                ...
                <div className="flex">
                    <CustomDropdown
                        ...
                        autoFocus
                        name="selectedMsa"
                        labelId="organizationContractLayers.label"
                        options={companies.map((c) => ({
                            value: c.name,
                            label: c.name
                        }))}
                        required={required}
                        requiredMsg="organizationContractLayers.required"
                        placeholder="Selected Organization Name"
                        onChange={(e) => {handleDropdownChange(e.value);
                                          setValue('selectedMsa',e.value);
                                          setIsShown(false)
                                           }} // dropdown linked data
                        className="md:col-11"
                ...
                <div className="container mt-2">
                    ...
                    {/* p-text-secondary for lavel , p-text-primary */}
                     ...
                     <label className='p-text-primary'>Organization Name</label>
                     ...
                     <h6  className='p-text-secondary'>{sampleDynamicData?.name}</h6>
                        </div>
                        {/* <div className="col-sm-12 col-md-4 mb-1">
                            <label>EIN</label>
                        ...
                        <div className="col-sm-12 col-md-4 mb-1">
                            ...
                            <h6 className='p-text-secondary'>{sampleDynamicData?.taxClassification}</h6>
                        </div>
                        <div className="col-sm-12 col-md-4 mb-1">
                            <label className='p-text-primary'>Phone Number</label>
                            <h6 className='p-text-secondary'>{sampleDynamicData?.phoneNumber}</h6>
                        </div>
                        {/* <div className="col-sm-12 col-md-4 mb-1">
                            <label>Fax</label>
                            <h6>{sample?.phoneNumber}</h6>
...
export default SingleResourceWoAddOrganization;