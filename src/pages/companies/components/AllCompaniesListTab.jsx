...
const AllCompaniesListTab = ({
    ...
    const total = companyPagination.totalElements || 0;
    ...
    // const changeOrganizationStatus = async (organizationID, statusCode, inactiveStatusCode) => {
    //     // ... (other modifications remain the same)
    // };

    const onChangeStatusClick = async (event) => {
        try {
            const companyToChangeStatus = activeRowMenu || selectedCompany;

            if (!companyToChangeStatus) {
                console.error('No company selected to change status.');
                return;
            }

            // Set the status modal visibility to true
    ...
    //     try {
    //         // Dispatch the action to change organization status
    //         dispatch(changeOrganizationStatusRequest({
    //             organizationID: selectedCompany.organizationID,
    //             statusCode: 'INACTIVE',
    //             inactiveStatusCode: 'ONHOLD',
    //         }));
    //     } catch (error) {
    ...
    const handleStatusChange = async () => {
        try {
            ...
            dispatch(changeOrganizationStatusRequest({
                organizationID: selectedCompany.organizationID,
                statusCode: selectedStatus,  // Use the selected status from the dropdown
                inactiveStatusCode: 'ONHOLD',
            }));
        } catch (error) {
            console.error('Status Change:', error);
    ...
    const onRowClick = (event) => {
        // console.log("selectedRow11", event.index);
        ...
        <>
            ...
            <div>
                <Dialog
                    ...
                    modal
                    onHide={() => setStatusModalVisible(false)}
                >
                    <div>
                        ...
                        />
                        <div className='d-flex gap-2 mt-4 align-item-center'>
                            <Button label="Cancel" severity='secondary' size='small' onClick={() => setStatusModalVisible(false)} />
                            <Button label="Change" size='small' onClick={handleStatusChange} />
                        </div>
                    </div>
                </Dialog>
