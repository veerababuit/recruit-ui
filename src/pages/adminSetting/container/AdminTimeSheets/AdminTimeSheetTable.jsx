...
import { fetchTimeSheetDocumentRequest } from '../../../../redux/actions/adminResourceRoleAction';
...
function AdminTimeSheetTable({ handleRowSelect, handleRowUnselect, setActiveRowMenu, action, onRowClick }) {
    ...
    const { TimeSheetDocumentsList, loadingTimeSheetDocuments, error } = useSelector((state) => state.adminRole);

    useEffect(() => {
        dispatch(fetchTimeSheetDocumentRequest());
    }, [dispatch]);
    ...
    return (
        <div>
            ...
            {loadingTimeSheetDocuments && !error && TimeSheetDocumentsList.length === 0 && (
                <DocumentDefinitionTableSkeleton />
            )}
            {!loadingTimeSheetDocuments && TimeSheetDocumentsList.length > 0 && (
                <DataTable
                    value={TimeSheetDocumentsList}
                    dataKey="id"
